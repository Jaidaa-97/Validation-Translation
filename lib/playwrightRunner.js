...
function extractMatchingSnippet(pageText, expected) {
  ...
  const idx = normalizeTextForCompare(body).indexOf(normalizeTextForCompare(exp));
  if (idx === -1) {
    return null;
  }
...
function sanitizeDiningOutletRecord(outlet, expected = '') {
  ...
  const description = stripDiningQuotesUnlessInExpected(outlet?.description || '', expected);
  let text = stripDiningQuotesUnlessInExpected(outlet?.text || '', expected);
  ...
  try {
    ...
    for (const row of searchableRows) {
      ...
      const allowDiningOrdinalFallback =
        diningOrdinal > 0 && /restaurant\s*#?\s*\d+|\boutlet\s*#?\s*\d+/i.test(section);
      ...
      if (found) {
        ...
        if (useDiningName) {
          ...
          passNote =
            'Match verified in spa / wellness hours (schedule `<p>` on /services-amenities/spa, e.g. Weekdays + Fitness Center times).';
        } else if (usePH) {
