  ...
  const raw = await page
    .evaluate(() => {
      ...
      const looksLikeSpaHours = (text) => {
        ...
        if (
          /weekday|weekend|wochentag|wochenende|feriale|finesettimana|werktags|samstag|sonntag|sabato|domenica|sábado|domingo|lunes a viernes|nei giorni feriali|fitness|massage|sauna|hammam|thermal|treatment|wellness|\bspa\b|pool|gym|centre|center/i.test(
            lower,
          )
        ) {
          return true;
        ...
        return t.length <= 52 && /^[\p{L}\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\s./-]{2,36}\d{1,2}\s*:\s*\d{2}/u.test(t);
      };
...
function looksLikeSpaScheduleLine(text) {
  ...
  const lower = t.toLowerCase();
  if (
    /weekday|weekend|wochentag|wochenende|feriale|finesettimana|werktags|samstag|sonntag|sabato|domenica|sábado|domingo|lunes a viernes|nei giorni feriali|fitness|massage|sauna|hammam|thermal|treatment|wellness|\bspa\b|pool|gym|centre|center|sanitas/i.test(
      lower,
    )
  ) {
    return true;
  ...
  if (t.length <= 52 && /^[\p{L}\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\s./-]{2,36}\d{1,2}\s*:\s*\d{2}/u.test(t)) {
    return true;
  }
  return false;
}
...
async function readSpaHoursOnCurrentPage(page) {
  ...
  return page
    .evaluate(() => {
      ...
      const isSpaish = (t) =>
        /weekday|weekend|wochentag|fitness|massage|sauna|hammam|wellness|\bspa\b|pool|gym|werktags|samstag|sonntag|sabato|domenica|sábado|domingo|lunes a viernes|nei giorni feriali|平日|土曜日|日曜日|月曜日|火曜日|水曜日|木曜日|金曜日/i.test(
          t,
        );
      let best = '';
      /** @type {string[]} */
...
function looksLikeGolfHoursContent(text) {
  ...
  const lower = t.toLowerCase();
  return (
    /tee time|golf|course|monday|tuesday|wednesday|thursday|friday|saturday|sunday|daily|every day|weekday|weekend|montag|sonntag|lunes|domingo|feriale|sabato|domenica|sábado|werktag|samstag|月曜|日曜|平日|土曜|日曜/i.test(
      lower,
    ) && !looksLikeSpaScheduleLine(t)
  );
}
...
function isGolfHoursRowLabel(sectionLabel, expected = '', golfCourses = null) {
  ...
  if (looksLikeGolfHoursContent(exp)) {
    if (/\bgolf\b/i.test(label) || isGenericOperationHoursSectionLabel(sectionLabel)) {
      return !expectedMatchesDiningOutlet(null, exp) && !looksLikeSpaHoursContent(exp);
    }
  }
  return false;
}
    ...
    .evaluate(() => {
      ...
      const extractHours = (root) => {
        ...
        const h2 = [...hoursRoot.querySelectorAll('h2')].find((el) =>
          /hours of operation/i.test(clean(textFrom(el))),
        );
        ...
        for (const node of scope.querySelectorAll('p, div.hours, li')) {
          ...
          if (
            ...
            /monday|sunday|weekday|daily|every|lunes|domingo|werktag|feriale|sabato|domenica|月曜|日曜|平日|土曜|日曜/i.test(
              t,
            )
          ) {
            lines.push(t);
        ...
        if (!lines.length) {
          ...
          return block
            .split(/\n/)
            .map(clean)
            .filter((t) => t && /\d{1,2}\s*:\s*\d{2}/.test(t))
            .join('\n');
        }
        return lines.join('\n');
      };
 ...
 * @returns {Promise<{ block: string, courses: { outlets: { name: string, description: string, hours: string, text: string }[] } | null }>}
 */
async function readGolfServicesBlock(page, hotelPageUrl, languageCode) {
  ...
  try {
    ...
    if (!outlets.length) {
      ...
      return { block: generic, courses: null };
    }
    const block = outlets.map((o) => o.text).join('\n\n---\n\n');
    return { block, courses: { outlets } };
  } catch {
    return { block: '', courses: null };
  }
}
...
async function runComparison({
  ...
  const needsGolfAmenityOperationHoursBlock = searchableRows.some((r) => {
    ...
    if (isGenericOperationHoursSectionLabel(s)) {
      const exp = plainTextFromMaybeHtml(r.byLang?.[lang] || r.byLang?.ENG || '');
      return looksLikeGolfHoursContent(exp) && !looksLikeSpaHoursContent(exp);
    }
    return false;
  });
  ...
  try {
    ...
    if (needsGolfAmenityInfoBlock || needsGolfAmenityOperationHoursBlock) {
      // eslint-disable-next-line no-console
      console.log('[Runner] Capturing golf amenity tiles + hours (#hotel-amenities, one pass)…');
      let golfAmenityData = await readAllGolfAmenityData(page);
    ...
    if (needsGolfBlock && golfRes) {
      ...
      if (
        (needsGolfAmenityInfoBlock || needsGolfAmenityOperationHoursBlock) &&
        isGolfAmenityDataEmpty({ items: golfAmenityInfoMap, operationHours: golfAmenityOperationHours })
      ) {
        try {
          const fromGolfPage = golfRes.amenityData || (await scrapeGolfAmenityDataOnPage(page));
          if (!normalizeText(golfAmenityInfoMap.golfLessons) && normalizeText(fromGolfPage?.items?.golfLessons)) {
            golfAmenityInfoMap.golfLessons = fromGolfPage.items.golfLessons;
          }
          if (!normalizeText(golfAmenityInfoMap.dressCode) && normalizeText(fromGolfPage?.items?.dressCode)) {
            golfAmenityInfoMap.dressCode = fromGolfPage.items.dressCode;
          }
          if (!normalizeText(golfAmenityInfoMap.handicapMen) && normalizeText(fromGolfPage?.items?.handicapMen)) {
            golfAmenityInfoMap.handicapMen = fromGolfPage.items.handicapMen;
          }
          if (!normalizeText(golfAmenityInfoMap.handicapWomen) && normalizeText(fromGolfPage?.items?.handicapWomen)) {
            golfAmenityInfoMap.handicapWomen = fromGolfPage.items.handicapWomen;
          }
          if (!normalizeText(golfAmenityOperationHours) && normalizeText(fromGolfPage?.operationHours)) {
            golfAmenityOperationHours = fromGolfPage.operationHours;
          }
        } catch {
          // ignore
        }
    ...
    for (const row of searchableRows) {
      ...
      const isGolfHoursRow = isGolfHoursRowLabel(section, expected, golfCourseBlocks);
      const isHoursRow =
      ...
      const sheetRestaurantCount = diningRowContextByOrdinal.size;
      const scrapedOutletCount = diningOutletBlocks?.outlets?.length || 0;
      const allowDiningOrdinalFallback =
        effectiveDiningOrdinal > 0 &&
        isDiningOutletTitleRow &&
        sheetRestaurantCount > 0 &&
        scrapedOutletCount === sheetRestaurantCount;
      ...
      if (
        ...
        if (bindOrdinal > 0) {
          if (sectionLooksLikeRestaurantDescription) {
            ...
            const matchedByTitle = [
              ...
              (candidate) =>
                textsMatchOutletTitle(diningOutletForRow.name, candidate) ||
                outletNamesLikelyMatch(diningOutletForRow.name, candidate),
            );
            if (matchedByTitle) {
              bindDiningOutletOrdinal(
                diningOutletBlocks,
      ...
      const golfHoursFromAmenities = normalizeOperationHours(golfAmenityOperationHours || '');
      const golfHoursTextForRow = isGolfHoursRow
        ? golfOrdinalFromLabel(section) > 0 && normalizeText(golfHoursFromCourse)
          ? golfHoursFromCourse
          : golfHoursFromAmenities || golfHoursFromCourse
        : '';
