...
function normalizeOperationHoursForCompare(text) {
  ...
  s = s.replace(/\b(breakfast|brunch|lunch|dinner|weekends?)\s*:\s*/gi, '$1 ');
  s = s.replace(
...
function textsMatchOperationHours(actual, expected) {
  ...
  const timeTokens = (s) =>
    [...s.matchAll(/\d{1,2}\s*:\s*\d{2}/g)]
      .map((m) => m[0].replace(/\s/g, ''))
      .sort()
      .join('|');
  if (timeTokens(a) && timeTokens(a) === timeTokens(e)) {
    const meals = ['breakfast', 'brunch', 'lunch', 'dinner'];
    const aMeals = meals.filter((m) => a.includes(m));
    const eMeals = meals.filter((m) => e.includes(m));
    if (aMeals.length && aMeals.length === eMeals.length && aMeals.every((m) => e.includes(m))) {
      return true;
    }
  }
  return false;
