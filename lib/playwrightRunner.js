 ...
 * @returns {string}
 */
function pickOperationHoursText(operationHours, ordinal = 0, sectionLabel = '') {
  ...
  const label = normalizeSectionLabel(sectionLabel).toLowerCase();
...
async function runComparison({ excelPath, languageCode, pageUrl, headless = true }) {
  ...
  const needsDiningOperationHoursBlock = rows.some(
    (r) => isDiningOperationHoursSectionLabel(r.section) && rowHasAnyLanguageContent(r),
  );
