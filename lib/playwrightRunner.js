...
/**
 * Map a [start, end) range in normalized compare text back to the original string.
 * @param {string} original
 * @param {number} normStart
 * @param {number} [normEnd]
 * @returns {string}
 */
function mapNormSliceToOriginal(original, normStart, normEnd = Infinity) {
  let startIdx = 0;
  let endIdx = String(original || '').length;
  for (let i = 0; i <= endIdx; i++) {
    const normLen = normalizeTextForCompare(String(original || '').slice(0, i)).length;
    if (!startIdx && normLen >= normStart) {
      startIdx = i;
    }
    if (Number.isFinite(normEnd) && normLen >= normEnd) {
      endIdx = i;
      break;
    }
  }
  return String(original || '').slice(startIdx, endIdx).trim();
}
...
function sanitizeDiningOutletRecord(outlet, expected = '') {
  ...
  const description = collapseStutteredDiningCopy(
...
async function runComparison({
  ...
  try {
    ...
    if (needsHotelPropertyOverviewBlock || needsPropertyOverviewSpecialBlock) {
      if (needsPropertyOverviewSpecialBlock) {
        ...
        // eslint-disable-next-line no-console
        console.log('[Runner] Capturing property-overview block for Overview row(s)…');
      }
      const overviewBlockText = await readPropertyOverviewSpecialBlock(page);
      if (needsHotelPropertyOverviewBlock) {
        hotelPropertyOverviewText = overviewBlockText;
      }
      if (needsPropertyOverviewSpecialBlock) {
        propertyOverviewSpecialText = overviewBlockText;
    ...
    for (const row of searchableRows) {
      ...
      if (found) {
        ...
        if (airportHint) {
          ...
          failNote =
            'Expected text not found. "Actual" is the hotel property overview (`section.property-overview`).';
