 ...
 * @returns {Promise<{ results: object[], diningMeta: object, spaMeta: object, propertySearchMeta: object, propertyOverviewSpecialMeta: object, propertyHighlightMeta: object, hotelName: string }>}
 */
async function runComparison({ excelPath, languageCode, pageUrl, headless = true }) {
  ...
  try {
    ...
    if (needsPropertyOverviewSpecialBlock || needsPropertyHighlightBlock) {
      await page
        .locator('#main-content')
        .first()
        .waitFor({ state: 'attached', timeout: 35_000 })
    ...
    for (const row of rows) {
      ...
      if (found) {
        ...
        const actualFallback = airportHint || diningHint || spaHint || specialHint || phHint || psHint;
