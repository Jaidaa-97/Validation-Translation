...
/**
 * Main description copy for Excel “Overview” rows: `<p>` blocks in the wide column of
 * `section.property-overview` (excludes the section title and left subheader blurb).
 * @param {import('playwright').Page} page
 * @returns {Promise<string>}
 */
      ...
      .evaluate(() => {
        const clean = (t) =>
          String(t || '')
            ...
            .trim();
        const textFrom = (el) =>
          clean(el?.innerText && String(el.innerText).trim() ? el.innerText : el?.textContent);
        ...
        const isSubheaderEl = (el) =>
          Boolean(
            el?.closest?.('.subheader, [class*="subheader"], .font-amiri') ||
              el?.matches?.('.subheader, [class*="subheader"], .font-amiri'),
          );
        ...
        const collectParagraphs = (root) => {
          ...
          return [...root.querySelectorAll('p')]
            .filter(
              (p) =>
                !p.closest('.section-header, .display-quote, .col-12.display-quote') &&
                !isSubheaderEl(p),
            )
            .map((p) => textFrom(p))
            .filter((t) => t.length > 30 && !isEventLinkLine(t));
        };
        ...
        for (const sel of [
          ...
          const paras = collectParagraphs(col);
          if (paras.length) {
            return paras.join('\n\n');
          }
        }
        ...
        if (bestParas.length) {
          return bestParas.join('\n\n');
        }

        const fallback = collectParagraphs(sec);
    ...
    for (const row of searchableRows) {
      ...
      if (found) {
        ...
        if (airportHint) {
          ...
          failNote =
            'Expected text not found. "Actual" is the property-overview description (main `<p>` copy, not the subheader).';
