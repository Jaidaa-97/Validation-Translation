...
/**
 * Lenient match for CMS / locale copy (collapsed whitespace, HTML in Excel).
 * @param {string} actual
 * @param {string} expected
 * @returns {boolean}
 */
function textsMatchExpected(actual, expected) {
  if (extractMatchingSnippet(actual, expected) !== null) {
    return true;
  }
  const body = normalizeText(actual);
  const exp = normalizeText(plainTextFromMaybeHtml(expected));
  if (!body || !exp) {
    return false;
  }
  if (body.includes(exp) || exp.includes(body)) {
    return true;
  }
...
function outletNamesLikelyMatch(actualName, wantedName) {
  ...
  return shared > 0 && shared === Math.min(actualTokens.size, wantedTokens.size);
}
...
function pickDiningOutletBySignals(diningOutlets, signals = {}) {
  ...
  const names = [
    ...
    .map((name) => normalizeOutletNameForMatch(name))
    .filter((name) => name.length >= 3);

  for (const wanted of names) {
    for (const outlet of outlets) {
      if (outletNamesLikelyMatch(outlet.name, wanted)) {
        return outlet;
      }
    }
  }
  ...
  for (const description of descriptions) {
    ...
    for (const outlet of outlets) {
      const actual = normalizeText(outlet.text);
  ...
  if (expectedHours.length >= 4) {
    for (const outlet of outlets) {
      const actualHours = normalizeText(outlet.hours || '');
      const actualText = normalizeText(outlet.text);
      if (
        actualHours.includes(expectedHours) ||
        expectedHours.includes(actualHours) ||
        actualText.includes(expectedHours)
      ) {
        return outlet;
      }
    }
 ...
 * @returns {string}
 */
function pickDiningOutletText(diningOutlets, ordinal = 0, sectionLabel = '', preferredOutletName = '') {
  const outlet = pickDiningOutletBySignals(diningOutlets, {
    ordinal,
    sectionLabel,
    names: [preferredOutletName],
  });
  return outlet?.text || '';
}
  ...
  let outlets = await page
    .evaluate((hoursSelector) => {
      ...
      const descriptionFor = (blockText, name, hoursText, hoursEl) => {
        ...
        if (card) {
          const paragraphText = [...card.querySelectorAll('p')]
            .filter((p) => !p.closest('.hours-of-operation'))
      ...
      for (const hoursEl of document.querySelectorAll(hoursSelector)) {
        const name = outletNameFor(hoursEl) || `Dining outlet ${collected.length + 1}`;
  ...
  try {
    ...
    for (const row of searchableRows) {
      ...
      if (sectionLooksLikeRestaurantName) {
        const outletNameForMatching = expected || plainTextFromMaybeHtml(row.byLang.ENG || '');
        if (normalizeText(outletNameForMatching)) {
          currentDiningOutletName = outletNameForMatching;
        }
      }
      ...
      const diningNameTextForRow = sectionLooksLikeRestaurantName
        ? String(diningOutletForRow?.name || '').trim()
        : '';
      ...
      let hoursText = '';
      if (isHoursRow) {
        let hoursOrdinal = diningOrdinal;
        hoursText = normalizeOperationHours(diningOutletForRow?.hours || '');
        if (!normalizeText(hoursText) && diningOutletForRow?.name) {
          hoursText = pickOperationHoursText(operationHours, 0, section, diningOutletForRow.name);
        }
      ...
      if (useAirport) {
        ...
        textForCompare = diningTextForRow || diningBlock;
      } else if (useSpa) {
      ...
      const found =
        useSpecialNotice || useOperationHours || useDiningDescription || useMessageBanner
          ? textsMatchExpected(textForCompare, expected)
          : snippet !== null && bodyNorm.includes(expNorm);
      ...
      if (found) {
        ...
        let passNote = '';
        if (useDiningName) {
          passNote = 'Restaurant name matched against the dining subpage outlet heading.';
        } else if (useDiningDescription) {
          passNote = 'Restaurant description matched against the dining subpage outlet description.';
        } else if (useDining) {
          ...
          passNote =
            specialNoticeMessages.length > 1
              ? `Match verified in special-notice block ${noticeIdx} of ${specialNoticeMessages.length} (\`div.alert.alert-info.special-notice div.message\`).`
        ...
        const diningDescriptionHint = diningDescriptionHintRaw
          ...
          : '';
        const diningHintRaw = useDining && (diningTextForRow || diningBlock)
          ? normalizeText(diningTextForRow || diningBlock)
          : '';
