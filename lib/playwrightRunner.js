...
/**
 * Some LHW spa pages embed hours inside `.spa-details` as `<br>` / `<b>` lines (no `<p>`).
 * @param {string} text
 * @returns {{ description: string, hours: string }}
 */
function splitSpaDescriptionAndEmbeddedHours(text) {
  const source = String(text || '').trim();
  if (!source) {
    return { description: '', hours: '' };
  }
  const markerRe =
    /\n\s*(?:hours?\s+of\s+operations?|opening\s+hours?|operating\s+hours?|heures?\s+d['']?ouverture|horaires?\s*(?:d['']?ouverture)?|horario(?:s)?|horários?|öffnungszeiten|apertura|orari(?:\s+di\s+apertura)?)\s*:?\s*\n/i;
  const m = source.match(markerRe);
  if (m && typeof m.index === 'number' && m.index > 0) {
    return {
      description: source.slice(0, m.index).trim(),
      hours: source.slice(m.index + m[0].length).trim(),
    };
  }
  return { description: source, hours: '' };
}
 ...
 * @returns {string}
 */
function pickLongerSpaHoursText(primary, secondary) {
  const a = normalizeSpaHoursText(primary);
  const b = normalizeSpaHoursText(secondary);
  if (!a) {
    return b;
  }
  if (!b) {
...
async function extractSpaTitleAndDescriptionParts(page) {
  ...
  const split = splitSpaDescriptionAndEmbeddedHours(description);
  description = split.description;
  const hours = pickLongerSpaHoursText(raw?.hours || '', split.hours);
...
/**
 * LHW often stores the spa blurb and “Hours of operations” in one Excel “Spa Description” cell.
 * @param {string} spaDescription
 * @param {string} [spaHours]
 * @returns {string}
 */
function buildSpaDescriptionCompareText(spaDescription, spaHours = '') {
  const desc = String(spaDescription || '').trim();
  const hrs = normalizeSpaHoursText(spaHours);
  if (!desc) {
    return hrs;
  }
  if (!hrs) {
  ...
  const descCmp = normalizeTextForCompare(desc);
  const hrsCmp = normalizeTextForCompare(hrs);
  if (descCmp.includes(hrsCmp)) {
    return desc;
  }
  return `${desc}\n\nHours of operations:\n${hrs}`;
}
...
function isSpaHoursRowLabel(
  ...
  if (isSpaDescriptionSectionLabel(sectionLabel) || isSpaTitleSectionLabel(sectionLabel)) {
    return false;
  }
...
function pickSpaTextForRow(sectionLabel, spaBlock, spaTitle, spaDescription, spaHours = '') {
  ...
  if (isSpaDescriptionSectionLabel(sectionLabel)) {
    return buildSpaDescriptionCompareText(spaDescription, spaHours);
  }
...
async function runComparison({
  ...
  /**
   * Merged expected features for the SELECTED language. Empty when no feature row had content
   * for this language (or no feature rows exist at all).
  ...
  try {
    ...
    for (const row of searchableRows) {
      ...
      if (useAirport) {
        // Compare only p.airport — not the full overview (length guard would always fail).
        textForCompare = airportBlock;
      ...
      if (found) {
        ...
        if (useSpecialOverview) {
          ...
          actualPass = truncateForResultPreview(
            buildSpaDescriptionCompareText(spaDescriptionForMeta, spaHoursForMeta) || spaTextForRow,
            12_000,
          );
        } else if (useSpa) {
        ...
        const spaHintSource =
          isSpaTitleSectionLabel(section) && normalizeText(spaTitleForMeta)
            ? spaTitleForMeta
            : isSpaDescriptionSectionLabel(section)
              ? buildSpaDescriptionCompareText(spaDescriptionForMeta, spaHoursForMeta)
              : spaTextForRow;
    ...
    // Per-feature comparison output (replaces the single blob row for feature section labels).
