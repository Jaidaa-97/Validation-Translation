...
function looksLikeGolfHoursContent(text) {
  ...
  if (
    /tee time|golf|course|monday|tuesday|wednesday|thursday|friday|saturday|sunday|daily|every day|weekday|weekend|montag|sonntag|lunes|domingo|feriale|sabato|domenica|sábado|werktag|samstag|lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche|月曜|日曜|平日|土曜|日曜/i.test(
      lower,
    ) &&
...
function shouldUseGolfOperationHours(expected, ctx) {
  ...
  if (looksLikeGolfHoursContent(exp)) {
    const spa = pickBestSpaHoursText(ctx.spaHours || '', ctx.hotelAmenitiesHours || '');
    if (!normalizeText(spa) || !textsMatchOperationHours(spa, exp)) {
      return true;
    }
  }
  return false;
...
function searchableRowNeedsGolfSubpage(row, lang) {
  ...
  if (isGenericOperationHoursSectionLabel(section) && looksLikeGolfHoursContent(expected)) {
    return true;
  }
  return false;
}
...
async function runComparison({
  ...
  const needsGolfAmenityOperationHoursBlock = searchableRows.some((r) => {
    ...
    if (isGenericOperationHoursSectionLabel(s)) {
      ...
      return looksLikeGolfHoursContent(exp);
    }
    return false;
  });
  ...
  try {
    ...
    for (const row of searchableRows) {
      ...
      // When sheet labels explicitly say "Restaurant #N ...", trust that index first.
      if (
        !diningOutletForRow &&
        diningOrdinal > 0 &&
        diningOutletBlocks?.outlets?.[diningOrdinal - 1]
      ) {
        diningOutletForRow = diningOutletBlocks.outlets[diningOrdinal - 1];
      }
