...
function outletNamesLikelyMatch(actualName, wantedName) {
  ...
  if (shared >= 2 && shared >= minDistinct - 1) {
    return true;
  }
...
function pickDiningOutletBySignals(diningOutlets, signals = {}) {
  ...
  if (matchByTitle) {
    ...
    if (labelHint) {
      for (const outlet of outlets) {
        if (outletNamesLikelyMatch(outlet.name, labelHint)) {
          return outlet;
        }
      }
    }
    ...
    if (normalizeText(expectedName)) {
      for (const outlet of outlets) {
        if (outletNamesLikelyMatch(outlet.name, expectedName)) {
          return outlet;
        }
      }
    }
    ...
    for (const row of searchableRows) {
      ...
      if (!diningOutletForRow) {
        diningOutletForRow = pickDiningOutletBySignals(diningOutletBlocks, {
          ...
          matchByTitle: isDiningOutletTitleRow && !isHoursRow,
          matchOutletNameOnly: isDiningOutletTitleRow && !isHoursRow,
        });
      }
      ...
      if (sectionLooksLikeRestaurantName && diningOutletForRow) {
        const bindOrdinal = diningHoursOrdinalFromLabel(section) || effectiveDiningOrdinal;
        const matchedByTitle = [...diningTitleCandidates.localizedNames, ...diningTitleCandidates.englishNames].some(
          (candidate) => outletNamesLikelyMatch(diningOutletForRow.name, candidate),
        );
        if (bindOrdinal > 0 && matchedByTitle) {
