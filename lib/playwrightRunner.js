...
function pickDiningOutletBySignals(diningOutlets, signals = {}) {
  ...
  if (matchByTitle) {
    const wantedNames = [
      ...
      diningOutletNameHintsFromSectionLabel(signals.sectionLabel || ''),
...
async function scrapeDiningOutletsFromPage(page) {
  ...
  return page
    .evaluate((hoursSelector) => {
      ...
      const isTitleHeading = (el) =>
        Boolean(
          el?.matches?.(
            'h1,h2,h3,h4,h5,h6,.restaurant-name,.dining-title,[class*="restaurant-name"]',
          ),
        );
      const hasOutletSignals = (scope, hoursSelector) => {
        if (!scope) {
          return false;
        }
        if (scope.querySelector?.(hoursSelector)) {
          return true;
        }
        return [...(scope.querySelectorAll?.('p') || [])].some(
          (p) => !isInsideDiningQuote(p) && cleanLine(p.innerText || p.textContent).length > 25,
        );
      };
      const sectionRootForTitle = (titleEl, nextTitleEl, hoursSelector) => {
        let best = titleEl.parentElement;
        for (let cur = titleEl.parentElement; cur; cur = cur.parentElement) {
          if (cur === document.body || cur === document.documentElement) {
            break;
          }
          if (nextTitleEl && cur.contains(nextTitleEl) && cur !== titleEl.parentElement) {
            break;
          }
          best = cur;
        ...
        return best || titleEl.parentElement;
      };
      const descriptionFromSection = (sectionRoot, titleName, hoursEl, hoursSelector) => {
      ...
      /** @type {{ el: Element, name: string }[]} */
      const titleCandidates = [];
      for (const el of root.querySelectorAll(
        'h1,h2,h3,h4,h5,h6,.restaurant-name,.dining-title,[class*="restaurant-name"]',
      )) {
        const name = cleanLine(el.innerText || el.textContent);
        if (skipRestaurantTitle(name)) {
          continue;
        }
        const localScope = el.parentElement;
        let nearby = hasOutletSignals(localScope, hoursSelector);
        if (!nearby) {
          let sib = el.nextElementSibling;
          for (let i = 0; i < 8 && sib; i++) {
            if (isTitleHeading(sib) && cleanLine(sib.innerText || sib.textContent).length >= 2) {
              break;
            }
            if (hasOutletSignals(sib, hoursSelector)) {
              nearby = true;
              break;
            }
        ...
        titleCandidates.push({ el, name });
      }

      const titles = titleCandidates.filter(
        (item, idx) =>
          !titleCandidates.some(
            (other, j) => j !== idx && item.el !== other.el && item.el.contains(other.el),
          ),
      );
      ...
      for (let i = 0; i < titles.length; i++) {
        const { el: titleEl, name } = titles[i];
        const nextTitleEl = titles[i + 1]?.el || null;
        const sectionRoot = sectionRootForTitle(titleEl, nextTitleEl, hoursSelector);
        const hoursEl = sectionRoot?.querySelector?.(hoursSelector) || null;
        const hours = hoursEl ? normalizeHours(hoursEl.innerText || hoursEl.textContent) : '';
        const description = descriptionFromSection(sectionRoot || titleEl.parentElement, name, hoursEl, hoursSelector);
...
function normalizeOperationHoursForCompare(text) {
  let s = normalizeOperationHours(text);
  s = normalizeText(s).toLowerCase();
  s = s.replace(
    /\b(breakfast|brunch|lunch|dinner|weekends?|weekdays?|weekday|fitness|massage|pool|gym|sauna|hammam|wellness|spa)\s*:\s*/gi,
    '$1 ',
  );
  s = s.replace(
    /\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday)(?:\s*-\s*(monday|tuesday|wednesday|thursday|friday|saturday|sunday))?\s*:\s*/gi,
    '$1$2 ',
  );
  s = s.replace(/\b(last order|late night|until late)\s*:\s*/gi, '$1 ');
...
function operationHoursTimeTokens(s) {
  return [...String(s || '').matchAll(/\d{1,2}\s*:\s*\d{2}/g)]
  ...
  try {
    ...
    for (const row of searchableRows) {
      ...
      const diningRowContext = effectiveDiningOrdinal
        ? diningRowContextByOrdinal.get(effectiveDiningOrdinal) || { names: [], descriptions: [] }
        : { names: [], descriptions: [] };
      ...
      const diningOutletForRow = pickDiningOutletBySignals(diningOutletBlocks, {
        ...
        descriptions: [],
        expectedHours: '',
        expectedText: sectionLooksLikeRestaurantName ? expected : '',
        allowOrdinalFallback: allowDiningOrdinalFallback,
        matchByTitle: isDiningOutletTitleRow,
        matchOutletNameOnly: isDiningOutletTitleRow,
      });
      ...
      const useDining =
        ...
        !useHotelPropertyOverview &&
        !useHotelPropertyQuote &&
        (isRestRow || (diningPathPasted && !isSpaRow && !isHotelOverviewRow && !isHotelQuoteRow));
      const useSpaHours =
      ...
      const useStrictCopyMatch =
        useHotelPropertyOverview || useHotelPropertyQuote || useDiningDescription;
      const useHoursMatch = useOperationHours || useSpaHours;
