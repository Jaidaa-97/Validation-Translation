 ...
 * @returns {Promise<string>}
 */
async function readServicesAmenitiesBlock(page, hotelPageUrl, languageCode, amenity, spaPartOut = null) {
  const targetUrl =
    amenity === 'spa'
 ...
 * @returns {Promise<{ results: object[], diningMeta: object, spaMeta: object, propertySearchMeta: object, propertyOverviewSpecialMeta: object, propertyHighlightMeta: object, messageBannerMeta: object, specialNoticeMessage: string|null, hotelName: string }>}
 */
async function runComparison({ excelPath, languageCode, pageUrl, headless = true }) {
  ...
  console.log(`[Runner] Starting comparison (language=${lang}, rows=${rows.length}, url=${pageUrl})`);
  ...
  const needsPropertyOverviewSpecialBlock = rows.some((r) => rowHasSpecialNote(r));
  ...
  const needsHotelMessageBannerBlock =
    rows.some((r) => isHotelMessageBannerSectionLabel(r.section) && rowHasAnyLanguageContent(r));
  const needsSpecialNoticeBlock =
    rows.some((r) => isSpecialNoticeSectionLabel(r.section) && rowHasAnyLanguageContent(r));
  // True when at least one of the 4 feature-category rows has a non-empty cell for ANY language.
  ...
  const hasAnyFeatureRow = rows.some(
    (r) => isFeatureSectionLabel(r.section) && rowHasAnyLanguageContent(r),
  );
  ...
  const needsHighlightTitlesBlock = rows.some((r) =>
   ...
   */
  const expectedFeatures = hasAnyFeatureRow ? buildExpectedFeatures(rows, lang) : [];
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(`[Runner] Comparing ${rows.length} row(s) against the captured page text…`);
    ...
    for (const row of rows) {
      ...
      if (found) {
        ...
        } else if ((useDining || useSpa || usePH || usePS) && (snippet || expNorm)) {
        ...
        if (useDining) {
          ...
          passNote =
            'Match verified on the spa subpage (/services-amenities/spa). Pool includes SPA title (#spa-list-title), description (.spa-details / fallbacks), and optional generic body text when the structured block was short.';
        } else if (useSpecialOverview) {
          passNote =
