 ...
 *   1. Picks feature rows from the uploaded Excel CONTENT sheet by their column-A label
 *      (Recreation & Health Complimentary / Additional Cost, General Features Complimentary /
 *      Additional cost, combined Health Features / General Features — case-insensitive).
...
const { normalizeText, plainTextFromMaybeHtml } = require('./textNormalize');
...
/**
 * Returns true when the given column-A label is one of the feature categories
 * (General / Recreation & Health rows, or the combined “Health Features” / “General Features” rows).
...
function normalizeFeatureText(text) {
  return normalizeText(plainTextFromMaybeHtml(text)).toLowerCase();
}
...
function splitFeatureCell(cellText) {
  const raw = plainTextFromMaybeHtml(cellText);
  if (!raw.trim()) {
    return [];
  }
  ...
  const parts = raw
    .split(/\r?\n+/)
    .map((p) => plainTextFromMaybeHtml(p).trim())
    .filter((p) => p.length > 0);
...
/**
 * Build the merged `expectedFeatures` array for one language across all feature rows.
 *
 ...
 *
 * Dedupes within each Excel row (case-insensitive). The same bullet in two different
 * feature rows (e.g. Health Features and General Features) is still compared twice so
 * both sections are validated when the workbook has values in each.
 *
...
function buildExpectedFeatures(rows, languageCode) {
  ...
  for (const row of rows) {
    ...
    const labelKey = normalizeText(row.section).toLowerCase();
    if (!seenByLabel.has(labelKey)) {
      seenByLabel.set(labelKey, new Set());
    }
    const seen = seenByLabel.get(labelKey);
 ...
 */
/**
 * Plain-text line from the live features block that best matches one expected item.
 * @param {string} actualText
 * @param {string} feature
 * @returns {string}
 */
function featureActualSnippet(actualText, feature) {
  const featureNorm = normalizeFeatureText(feature);
  if (!featureNorm) {
    return '';
  }
  for (const line of String(actualText || '').split(/\r?\n+/)) {
    const plain = plainTextFromMaybeHtml(line).trim();
    if (!plain) {
      continue;
    }
    if (normalizeFeatureText(plain).includes(featureNorm) || featureNorm.includes(normalizeFeatureText(plain))) {
      return plain;
    }
  }
  const actualNorm = normalizeFeatureText(actualText);
  if (actualNorm.includes(featureNorm)) {
    return plainTextFromMaybeHtml(feature).trim();
  }
  return '';
}
...
function compareFeatures(expectedFeatures, actualText) {
  ...
  return expectedFeatures.map(({ feature, sourceLabel }) => {
    const plainFeature = plainTextFromMaybeHtml(feature).trim();
    const featureNorm = normalizeFeatureText(plainFeature);
    const found = featureNorm.length > 0 && actualNorm.includes(featureNorm);
    return {
      feature: plainFeature,
      sourceLabel,
      found,
      actualSnippet: found ? featureActualSnippet(actualText, plainFeature) : '',
    };
  });
}
