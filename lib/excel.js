 ...
 * A=0 label, B=1 ENG, C=2 GER, D=3 ITA, E=4 FRE, F=5 JAP, G=6 SPA, H=7 Special note (optional flag — see runner)
 */
const COL = {
  LABEL: 0,
  ENG: 1,
...
/**
 * Trim + strip BOM / zero-width chars (Excel paste / export noise).
 * @param {string} str
 * @returns {string}
 */
 ...
 * @returns {{ section: string, byLang: Record<string, string>, specialNote: string }[]}
 */
function readContentSheet(filePath) {
  ...
  /** @type {{ section: string, byLang: Record<string, string>, specialNote: string }[]} */
  const out = [];

  for (let i = start; i < rows.length; i++) {
    const row = rows[i] || [];
    const section = cleanCell(row[COL.LABEL]);
    if (!section) {
      continue;
    }
    ...
    for (const code of Object.keys(LANG_TO_COL)) {
      ...
      byLang[code] = cleanCell(row[colIdx] ?? '');
    }

    const specialNote = cleanCell(row[COL.SPECIAL_NOTE] ?? '');

    out.push({ section, byLang, specialNote });
  }
