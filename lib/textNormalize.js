...
/**
 * Turn Excel HTML fragments into plain visible text for substring checks.
 * @param {string} text
 * @returns {string}
 */
/**
 * Normalize text for comparisons (case-insensitive; whitespace/HTML rules from `normalizeText`).
 * @param {string} text
 * @returns {string}
 */
...
/**
 * Case-insensitive substring check after `normalizeText` / HTML stripping.
 * @param {string} haystack
 * @param {string} needle
 * @returns {boolean}
 */
function compareTextIncludes(haystack, needle) {
  const h = normalizeTextForCompare(haystack);
  const n = normalizeTextForCompare(plainTextFromMaybeHtml(needle));
  if (!h || !n) {
    return false;
  }
