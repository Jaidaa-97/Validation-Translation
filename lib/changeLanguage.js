...
/**
 * Same pattern as dining/spa, for golf course copy on the golf subpage.
 *
 * @param {string} urlString
 * @param {string} languageCode
 * @returns {string|null}
 */
function buildGolfServicesUrl(urlString, languageCode) {
  return buildServicesAmenitiesUrl(urlString, languageCode, 'golf');
}
...
/**
 * True when the path is the golf subpage (user pasted a golf deep link).
 * @param {string} urlString
 * @returns {boolean}
 */
function isGolfServicesPath(urlString) {
  try {
    const u = new URL(urlString.trim());
    return /\/services-amenities\/golf\/?$/i.test(u.pathname);
  } catch {
    return false;
  }
}
